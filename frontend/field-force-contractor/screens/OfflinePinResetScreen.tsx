...
export default function OfflinePinResetScreen() {
  ...
  const [submitted, setSubmitted] = useState(false); // controls which view is shown
  ...
  const handleSubmit = () => {
    ...
    // Show the confirmation view instead of the form
    setSubmitted(true);
  }
    ...
    <MainFrame header="default" headerMenu={['none']} footerMenu={['none']}>
      ...
      >
        ...
        >
          ...
          {/* Switch between the form and the confirmation based on submitted state */}
          {submitted === false ? (

            // ── FORM VIEW ─────────────────────────────────────────
            <View style={styles.formView}>

              {/* Lock icon with a wifi-off badge to visually communicate "offline PIN" */}
              <View style={styles.illustrationWrap}>
                <View style={styles.illustrationCircle}>
                  <Ionicons name="lock-closed" size={48} color={colors.textWhite} />
                </View>
                {/* The small badge sits in the bottom-right corner of the circle */}
                <View style={styles.wifiOffBadge}>
                  <Ionicons name="wifi-outline" size={16} color={colors.warning} />
                </View>
              </View>

              {/* Explanation box — tells the user what's going on */}
            ...
            // ── CONFIRMATION VIEW ──────────────────────────────────
            // Shown after the user successfully submits their email
            <View style={styles.confirmView}>
              ...
              {/* Takes the user back to the offline login screen */}
              <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  // position: 'absolute' places it on top of the parent View
  wifiOffBadge: {
    position:        'absolute',
    bottom:          0,
    right:           -4,
